...
netG = wgans_kd.GeneratorKD(ngpu=1, num_classes=num_classes, z_dim=z_dim, latent_size=200, img_channel=1).to(dev)
...
reporter.set_experiment_name("Generator DC-GANs")
reporter.append_summary_description("Mainly experiment on Generator: based on DC-GANs")
...
try:
    ...
    for e in range(EPOCHS):
        ...
        for i, (fmri, img, label_idx) in enumerate(ld):
            ...
            for _ in range(d_epoch_steps):
                ...
                p_stem, l_01, l_02, l_03, l_04, l_05, fake_img = netG(zz, ly_p_idx, fy_p)
            ...
            for fmri_val, img_val, label_idx_val in ld_val:
                ...
                if i == 0 and (e + 1) % export_gen_img_every == 0:
                    ...
                    fy_p_img_exp = fy_p[0:preview_gen_num, :]
                    ly_p_idx_img_exp = ly_p_idx[0:preview_gen_num]
                    p_stem, l_01, l_02, l_03, l_04, l_05, fake_img = netG(zz, ly_p_idx_img_exp, fy_p_img_exp)
                ...
                if i == 0 and (e + 1) % FID_CALC_FREQ == 0:
                    ...
                    if best['FIDIncep'] is None or best['FIDIncep'] > fid_score:
                        ...
                        reporter.log("  > Exporting Generator: netG_DCGANs.pth")
                        save_model(netG, MODEL_PATH, "netG_DCGANs.pth")

                        reporter.log("  > Exporting Discriminator: netD_DCGANs.pth")
                        save_model(netD, MODEL_PATH, "netD_DCGANs.pth")

                        reporter.log("  > Exporting Non-image FE: non_img_extr_DCGANs.pth")
                        save_model(non_img_extr, MODEL_PATH, "non_img_extr_DCGANs.pth")

                        # Let's see how well the image generation
                        reporter.log("A preview of generated image has been exported: {}".format(
                            GenCommonConfig.IMG_PREV_EXP_FNAME).format(e))
                        zz = torch.randn(preview_gen_num, z_dim, 1, 1, device=dev)
                        ...
                        fake_img = make_grid(fake_img, nrow=preview_gen_num, normalize=True)
                        img_grid = torch.cat((real_img, fake_img), 1)
                        save_image(img_grid, IMAGE_PATH + GenCommonConfig.IMG_PREV_EXP_FNAME.format(e), normalize=False)
    ...
    reporter.stop()
    reporter.write_summary(os.path.dirname(__file__), f_name="summary_gen_dcgans.txt")
