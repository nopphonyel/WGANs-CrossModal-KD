...
SUMMARY_F_NAME = "summary_fc10l.txt"
...
non_img_extr = SimpleFCExtractor(in_features=948, num_layers=10, num_classes=6, latent_idx=8, latent_size=200).to(dev)
...
reporter = Reporter(loss_logger, acc_logger, log_buffer_size=5)

reporter.set_experiment_name("CrossModal Classifier SimpleFC (10 layers)")
reporter.append_summary_description("Using fMRI-FE that based on SimpleFC (10 layers, latent@layer[8]).")
    ...
    for e in range(EPOCHS):
        ...
        for i, (fmri, img, label_idx) in enumerate(ld):
            ...
            for fmri_val, img_val, label_idx_val in ld_val:
                ...
                if max_acc_j2 < j2_acc:
                    max_acc_j2 = j2_acc
                    reporter.log("New high j2_acc detected! = %.2f" % j2_acc)
                    reporter.log("Exporting model: non_img_extr")
